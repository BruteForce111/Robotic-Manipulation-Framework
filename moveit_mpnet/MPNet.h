...
//         class MPNet : public base::Planner
//         {
//         public:
//             /** \brief Constructor */
//             MPNet(const base::SpaceInformationPtr &si, bool addIntermediateStates = false);
...
//             torch::Tensor loadNormalizedPointCloud(std::string fname);

//             void baxterToMoveIt(torch::Tensor baxter_state_tensor, base::State *moveit_state);

//             torch::Tensor moveItToBaxter(std::vector<float> moveit_state_vector);

//             torch::Tensor moveItToBaxterAndNormalize(std::vector<float> moveit_state_vector);

//             void baxterToMoveItAndUnnormalize(torch::Tensor baxter_state_tensor, base::State *moveit_state);

//             //adding MPNet test functions below 
//             // void testEncoder();
//             at::Tensor getEncoding();

//             // void testStartGoalTensor();
...
namespace ompl
{
    namespace geometric
    {
        ...
        {
        public:
            MPNet(const base::SpaceInformationPtr &si);
            ...
            */
            ...
            void baxterToMoveItAndUnnormalize(torch::Tensor baxter_state_tensor, base::State *moveit_state, const float jointRange[], int dim);

            std::vector<float> moveItToBaxter(std::vector<float> moveit_state_vector);

            // torch::Tensor normalizeVector(std::vector<float> unnormalized_vec, const float jointRange[]);
            std::vector<float> normalizeVector(std::vector<float> unnormalized_vec, const float jointRange[]);

            std::vector<float> moveItToBaxterAndNormalize(std::vector<float> moveit_state_vector, const float jointRange[], int dim);

            at::Tensor getEncoding();

            // void getStartGoalTensor(base::State *start_state, const base::State *goal_state, torch::Tensor *start_tensor, torch::Tensor *goal_tensor);

            torch::Tensor getStartGoalTensor(base::State *start_state, const base::State *goal_state, bool start_first, const float jointRange[], int dim);

            torch::Tensor getStartStartTensor(base::State *start_state, base::State *goal_state, bool start_first, const float jointRange[], int dim);

            // torch::Tensor stateToTensor(base::State *state);

            // torch::Tensor goalToTensor(const base::State *goal_state, const float jointRange[], int dim);

            void copyGoalState(const base::State *const_goal_state, base::State *goal_state, int dim);
            ...
            // std::vector<ompl::base::State *> testReplanPath(std::vector<base::State *> path, base::State *goal,
            //                                                               std::shared_ptr<torch::jit::script::Module> MLP, at::Tensor obs_enc);

            std::vector<base::State *> MPNetSolve();

            // std::vector<ompl::base::State *> lazyStateContraction(std::vector<base::State *> path, float step_size, int dim);
            std::vector<ompl::base::State *> lazyStateContraction(std::vector<base::State *> path);

            // bool feasibilityCheck(std::vector<base::State *> path, float step_size, int dim);
            bool feasibilityCheck(std::vector<base::State *> path);

            bool feasibilityCheckOMPL(std::vector<base::State *> path);

            bool steerTo(base::State *start_state, base::State *end_state, float step_size, int dim);

            // std::vector<ompl::base::State *> replanPath(std::vector<base::State *> path, base::State *goal,
            //                                                                 std::shared_ptr<torch::jit::script::Module> MLP, at::Tensor obs_enc, float step_size, int dim);
            std::vector<ompl::base::State *> replanPath(std::vector<base::State *> path, base::State *goal,
